import type { SampleContent } from './types';

const content: SampleContent = {
  title: 'Amsterdamse grachten',
  short: `De **Amsterdamse grachten** behoren tot het mooiste van Nederland. De stad wordt doorkruist door honderden grachten en bruggen. *“God schiep de wereld, maar de Hollanders schiepen Holland”*, zegt men. Een rondvaart is de beste manier om de stad te zien.`,
  long: `# De Amsterdamse grachten

De **Amsterdamse grachten** behoren tot het mooiste van Nederland. De stad wordt doorkruist door honderden grachten en bruggen. *“God schiep de wereld, maar de Hollanders schiepen Holland”*, zegt men.

## Geschiedenis

In de zeventiende eeuw groeide Amsterdam uit tot een wereldstad. De grachtengordel werd aangelegd als een halve maan rond de oude binnenstad. De grachten dienden voor handel, transport en verdediging.

## Wat te zien

- **De Herengracht** : de rijkste gracht met statige herenhuizen
- **De Jordaan** : een gezellige wijk met kleine winkels en cafés
- **De Negen Straatjes** : leuk om te winkelen

## Tips

- Maak een rondvaart door de grachten
- Huur een fiets om de stad op je eigen tempo te ontdekken

> Amsterdam is de stad van grachten, fietsen en vrijheid.`,
};

export default content;
