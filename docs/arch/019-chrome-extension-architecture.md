...
- **Status**: accepted
- **Created**: 2026-07-30
- **Last Updated**: 2026-07-30 (added translation pipeline & i18n architecture)
...
### Translation Pipeline & i18n Architecture

The extension's internationalization follows a strict pipeline that bridges the monorepo's `translations.csv` (single source of truth) with Chrome's `_locales/{locale}/messages.json` format. Understanding the `$` quirk and the dotted-key constraint is essential to avoid load failures and untranslated UI.

#### Data Flow

```
translations.csv  (monorepo source of truth, 31 locales)
      │
      ▼
generate-locales.js
      │  • CSV_LOOKUP: extension key → CSV key mapping
      │  • MANUAL: extension-only keys with hardcoded 31-locale translations
      │  • Converts {name} → $name$ placeholders (Chrome format)
      │  • Resolves dotted CSV keys to flat extension keys
      ▼
_locales/{locale}/messages.json  (31 files)
      │
      ▼
Chrome runtime → chrome.i18n.getMessage() / t() helper
```

#### The `$` Quirk (CRITICAL)

Chrome's `messages.json` requires **named placeholders** in this exact syntax:
...
1. **A bare `$1$` in the message with NO `placeholders` definition causes Chrome to reject the entire extension load** with `Variable $1$ used but not defined.` Every `$name$` in a message must have a corresponding entry under `placeholders`. The `content` field maps it to `$1`, `$2`, etc. (the substitution arguments passed to `chrome.i18n.getMessage()`).
2. **Dots (`.`) are not allowed in Chrome message keys.** CSV keys look like `subtitle.translating` or `msg.loading`, but you can never call `chrome.i18n.getMessage('subtitle.translating')` — Chrome rejects these. The source code must use flat extension keys (`translating`, `loadingSubtitles`) while `CSV_LOOKUP` maps them to the dotted CSV keys.

#### CSV_LOOKUP (extension key → CSV key)
...
Keys in `CSV_LOOKUP` pull their translations from the CSV for all 31 locales. Keys NOT in `CSV_LOOKUP` fall back to the `MANUAL` map (extension-only strings that have no CSV equivalent — e.g., `startPlaying`, `popupInstructions`, `extensionDescription`).

#### Placeholder Conversion in the Generator

Every value pulled from CSV uses `{name}` placeholder syntax (e.g., `Loading {lang} subtitles`). The generator converts these to Chrome's `$name$` format:

```js
translated = translated.replace(/\{(\w+)\}/g, '$$$1$');
```

The corresponding `placeholders` definition is preserved from the `en/messages.json` template entry, so the message resolves correctly at runtime.

#### Runtime Lookup (`t()` helper)

The `t()` helper in `src/i18n.js` has two lookup paths:

1. **Runtime cache first** — if `setLocale()` was called, it uses the fetched messages and manually replaces `$name$` via the `placeholders` config.
2. **`chrome.i18n.getMessage()` fallback** — preferred for placeholder resolution since Chrome natively handles named placeholders: `chrome.i18n.getMessage(key, ...substitutions)`.

#### Localized Language Names (L2 dropdown)

Language names in the L2 dropdown come from `dist/lang-names.json`, generated by `scripts/generate-lang-names.js` from the CSV's `lang.*` keys. Each language has entries for all 31 locales, so a Japanese L1 user sees `日本語`, `中国語`, `フランス語` — never `Japanese`, `Chinese`, `French`.

#### Verifying No English in UI

After any i18n change, verify that a non-English L1 (e.g., Japanese) has zero English fallbacks:

```bash
# Check every key is translated and no value equals its English equivalent
python3.10 -c """
import json
with open('apps/chrome-extension/_locales/ja/messages.json') as f:
    ja = json.load(f)
with open('apps/chrome-extension/_locales/en/messages.json') as f:
    en = json.load(f)
eng = [k for k in en if k != 'appName' and ja.get(k,{}).get('message') == en[k].get('message')]
print('English fallbacks:', eng or 'NONE')
"""
```

#### Common Pitfalls

- **Never use `$1$` bare in a message** without a matching `placeholders` entry — Chrome refuses to load the extension.
- **Never put dots in message keys** in source — the extension won't load. Always go through `CSV_LOOKUP`.
- **Always add a `placeholders` definition** in `en/messages.json` when a message contains `$name$`.
- **After editing `en/messages.json`**, always rerun `node scripts/generate-locales.js` to regenerate all 31 locale files.
- **A missing locale in a `MANUAL` entry** silently falls back to English — audit `MANUAL` coverage when adding a key.
