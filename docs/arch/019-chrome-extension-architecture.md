...
│   ├── content.js                     ← esbuild IIFE bundle of content-entry.js + React + shared packages
...
|---|---|---|
| `Alt+T` | Toggle panel visibility | `content-entry.js` — `keydown` listener on `document` |
| `Ctrl/Cmd+Shift+Y` | Toggle panel visibility (alternative) | Same listener |
| `↑` / `↓` (panel open) | Seek to previous/next cue | Same listener (only when panel is visible) |
