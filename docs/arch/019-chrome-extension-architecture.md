...
- **Status**: accepted
- **Created**: 2026-07-30
- **Last Updated**: 2026-08-31 (page reader tokenizes a wrapper's own paragraph runs — aozora-style bare `<div>` bodies with heading sub-blocks are no longer dropped by the leaf-only filter, and existing `<ruby>` is stripped before tokenizing so the page's reading doesn't double-render; side panel per-tab open/sub-tab state + a tokenize guard so a tab is only tokenized when the panel is open on it, plus page restore on close — see notes below)
...
- **Page-mode lifecycle**: tokenization requires both `panelOpenState.open === true` and `pageTranslationVisibility.open === true`. Switching tabs or closing the side panel calls `cleanup()`, restores each block's original HTML, clears token listeners/cache, and closes the webpage dictionary modal. **Closing the side panel on a page-translation tab restores the page to its original, untokenized state** via `panelOpenState {open:false}` → `cleanup()` → `restoreTokens()`.
- **Per-tab side-panel state + tokenize guard**: the background tracks the real panel open state (`chrome.sidePanel.onOpened`/`onClosed`) and resets it synchronously on `tabs.onActivated`, exposing `getSidePanelState`. The side panel only claims a tab for page-translation/subtitles when the panel is actually open on it (`panelOpen && st.open && st.tabId === tabId`), and goes neutral (no pull/poll/claim) on a tab change, so switching away from a page-translation tab no longer tokenizes the next page. Each browser tab remembers its own sub-tab (`sidePanelTabByTab`) and restores it when the panel reopens there. `chrome.sidePanel.open()` requires a user gesture, so the panel cannot be programmatically reopened on a tab switch-back; reopening via the icon/shortcut/token-click restores the per-tab sub-tab.
- **Leaf-block filter + wrapper's own text**: a page block is "nested" (a pure wrapper, skipped) only when it contains a *visible* descendant matching `BLOCK_SELECTOR`; hidden descendants do not make it nested. Without this, a wrapper with only hidden sub-blocks (e.g. YouTube's always-hidden paid-comment-chip `<div>` inside a comment body) was skipped, orphaning the comment's *visible* body text — the Korean comment body `<span>` was never tokenized while the author/Reply labels were. Tokenization, page-translation snapshots, and the scroll-in observer all use `hasVisibleBlockDescendant`. **A wrapper that ALSO owns its own paragraph text is not skipped**: `getTextRuns` stops at descendant block elements (`BLOCK_TAG_SET`), so a bare `<div class="main_text">` whose body paragraphs are direct children (aozora's `人間失格` — paragraphs split by `<br>`, heading `div.jisage_*`/`h3` sub-blocks) is tokenized for its own runs while its descendant blocks are tokenized by their own pass. Pure wrappers (all text inside descendant blocks) still yield no runs and are skipped. This fixes pages whose body text lives in a wrapper's direct text nodes (they were previously dropped entirely, so no tokens rendered).
- **Ruby stripping before tokenization**: before tokenizing a block, `stripRuby` replaces each `<ruby>` in the block's own runs with its `<rb>` base text (dropping `<rt>` readings and `<rp>` parens), after capturing `__lpvOriginalHtml` so restore keeps the original ruby-bearing HTML. This stops the page's own reading double-rendering over the tokenizer's fresh ruby (`buildRuby`) and removes stray `（）` parens. The read-only snapshot/context text paths also reject `<rp>` so a not-yet-tokenized block's translation text is clean (aozora uses ~391 `<ruby>`). The mid-flush abort path restores the block's original HTML so a close during a fetch never leaves the page ruby-less but untokenized.
...
- **Page translation block granularity + click highlight**: the Page Translation
  ...
  Token spans carry `data-lpv-run`, and a token **click** (`pageLookup`), carrying
  the run's `blockId` and `sentenceIndex`, scrolls the panel to that block and
  highlights the translation **sentence** that contains the token via
  `buildSentenceMap`/`sentenceIndexAt` (mirroring apps/web's
  `SentenceHighlightBlock`/`SegmentedTranslation`), rendered as a `primary`
  background wash on the active sentence. The rest of a block's translation text
  is rendered muted (`--muted-foreground`) so the highlighted sentence stands out.
  (Token hover does NOT highlight the translation — that was removed.)
