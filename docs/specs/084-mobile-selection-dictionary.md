...
- **Mobile plan**:
  ...
  3. Enable on subtitle **transcript** mode: the `TokenizedText` usage in `apps/mobile/components/video/SubtitleDisplay.tsx` **singleline** mode (line ~227). **Not** the multiline transcript (the FlatList rows): selectionDictionary is deliberately OFF there so non-ruby lines render per-token pressables and empty-space taps bubble to the row's seek `Pressable` (word taps still open the dictionary). For ruby/Japanese lines the native paragraph is used regardless (ruby rendering drives it, not selectionDictionary), so the paragraph's own tap handler now **ignores trailing empty-space taps** (`handleTap` returns when `offset == total`, `RubyTextParagraphView.swift`) — those no longer open the last token's popup; the row seeks instead. Not the on-video band (karaoke timing + selection conflict; web only enables transcript modes).
