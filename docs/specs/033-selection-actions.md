...
7. Touch devices (iPhone/iPad Safari): the hook detects touch (`ontouchstart` / `maxTouchPoints`) and switches from `mouseup`/`keyup` to a debounced `selectionchange` capture (400 ms after the selection settles, with `touchend` as a fallback trigger) so the popup opens after long-press + handle adjustment instead of mid-gesture. `TokenizedText` also applies `-webkit-touch-callout: none` on the selection-enabled container so iOS's native long-press callout doesn't fight the dictionary popup.
