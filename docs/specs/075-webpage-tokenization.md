...
- **Own-text runs for wrappers (aozora)**: a block that contains other block elements (a nested wrapper) is NOT skipped outright. `getTextRuns` stops at descendant block elements, so a wrapper that also owns its own paragraph text (e.g. aozora's bare `<div class="main_text">` whose body paragraphs are direct children split by `<br>`, alongside heading `div.jisage_*`/`h3` sub-blocks) is tokenized for those runs while its descendant blocks are tokenized by their own pass. Pure wrappers (all text inside descendant blocks) still produce no runs and are skipped.
- **Strip existing `<ruby>` before tokenizing**: `<ruby>/<rb>/<rt>/<rp>` in a block's own runs are replaced by the `<rb>` base text (dropping readings + parens), so the page's own ruby never double-renders over the tokenizer's fresh ruby.
