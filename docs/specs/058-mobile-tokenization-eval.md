...
(`scripts/tokenizer-eval/corpus_config.json`). The mobile suite reads from
`tmp/tokenizer-eval-mobile/corpus/`, which `prepare_fixtures.mjs` copies from
the SPEC-056 corpus (`tmp/tokenizer-eval/corpus/`) when missing. Everything
under `tmp/` is generated and gitignored — no corpus files are committed.

```
tmp/tokenizer-eval-mobile/
├── corpus/                  ← copied from SPEC-056 corpus (manifest + 19 .md)
├── fixtures/
│   ├── lemmas/{l2}.json
│   ├── dicts/{l2}.json
│   └── kuromoji/ja/, kuromoji/ko/
├── results/
│   ├── {l2}.json
│   ├── scorecard.json
│   └── scorecard.md
...
Refreshing the corpus is an explicit, reviewed act (re-run
`scripts/tokenizer-eval/fetch_corpus.py`, verify the richness rules, re-copy)
— never silent. `spot-checks.json` is tracked under
...
| **Dictionary hit rate** | 15% | Unique content tokens with an exact `head`/`alternate` match in the offline dictionary fixture ÷ unique content tokens. Full credit ≥ 50%, partial 30–49%, 0 below 30%. Applies when a dict fixture exists (default `zh`/`yue`/`th`); for languages without one the criterion is excluded and weights renormalize (note on scorecard). |
...
## First Run Snapshot & Findings (2026-08-09)

Source: `tmp/tokenizer-eval-mobile/results/scorecard.md`, regenerated by
`tokenizer-eval.test.ts` (sorted by total score, descending). Node/vitest,
main-thread chain, worker mocked.

| L2 | Tokens | Lemma cov. | Spot | Dict hit | Pron. | p95 (ms) | Total | Grade | Notes |
|---|---:|---:|---:|---:|---:|---:|---:|---:|---|
| en/fr/de/es/vi/ru/ar/tr/it/hi/id/nl/he/pt | 47–233 | 100% | all pass | — | 100% (ru/ar) | 0.1–1778 | 100 | A | dict criterion renormalized (no fixture) |
| zh | 93 | 100% | 1/1 | 94% | 96% | 156 | 99.6 | A | head + traditional alternate fixture |
| ja | 102 | 100% | 2/2 | — | 85% | 234 | 98.3 | A | kuromoji readings |
| th | 53 | 100% | 1/1 | 71% | 74% | 19 | 97.4 | A | dict-fixture gaps are data gaps |
| ko | 28 | 100% | 3/3 | — | 11% | 1222 | 89.5 | B | word-level romanization vs per-token surfaces |
| yue | 82 | 100% | 1/1 | 95% | 96% | 43 | 86.1 | B | pronunciation N/A main-thread (e2e covers worker) |

**Findings:**

- **Chinese fixture fidelity matters.** The first zh run scored 82.8 B
  because the dict fixture only contained simplified `head` forms while the
  corpus article is traditional script — dict max-matching fell back to
  char-by-char (avg token len 1.09). Expanding alternates into the fixture
  (mirroring the app's `head UNION alternate` query) made zh segment
  `語素/構成/語言/單位` identically to the device and score 99.6 A. Any future
  fixture regeneration must preserve both scripts.
- **Korean offline pronunciation coverage is a real gap (11%).** koroman
  romanizes word-level (`좋아합니다 → joahamnida`) but kuromoji-ko splits
  compounds into per-token surfaces (`좋아|합니다`), so attaching
  pronunciation per token misses most inflected forms. Lemmas are unaffected
  (3/3 spot checks pass). Follow-up tracked separately from this suite.
- **Dictionary coverage, not tokenization, is the binding constraint for
  scriptio-continua langs.** zh 94% / yue 95% / th 71% dict hit rates reflect
  the offline dictionaries themselves (e.g., `ภาษาไทย` and `動物學` are not
  headwords in the current th/yue exports).
- **Dict criterion renormalized** for the 16 languages without dict fixtures;
  generating dict fixtures for all 19 would make the scorecard directly
  comparable to SPEC-056's.
- **Latency numbers are warm-cache Node timings** (nl/pt/es p95 ≈ 1–1.8 s,
  ko ≈ 1.2 s), indicative only — real-device timings belong to e2e.
...
1. `prepare_fixtures.mjs` produces the corpus copy + fixtures for all 19 L2s
   (kuromoji packs for ja/ko, lemma tables for table languages, dict rows for
   zh/yue/th), all under `tmp/tokenizer-eval-mobile/`.
...
- Should CI cache the generated fixtures (e.g., a tar of
  `tmp/tokenizer-eval-mobile/fixtures/`) so CI doesn't require Flask, with
  full snapshots only for local scorecard runs? Or is the Flask-dependent
  prep acceptable (SPEC-056 already assumes a running server)?
