...
- **Image sources (mobile).** Sample content is authored once for both apps and points at web-root paths (`![travel](/travel.png)`), which a browser serves from `public/` but React Native cannot load. The mobile reader resolves those app-relative paths to their **bundled** asset (`apps/mobile/lib/reader-assets.ts` → `Image.resolveAssetSource`, which also yields the intrinsic size, so the block never waits on a size lookup). Any URI the reader cannot resolve — or that fails to load — must **settle**, never spin forever: the block falls back to the markdown alt text (and collapses when there is no alt text), with the failure logged. An image block must never leave an endless loading spinner on the page.
...
- **Plain (pre-tokenized) text matches the tokenized render.** While turning a page the reader shows plain text until the new blocks' tokens arrive. That plain state must render through the **same native paragraph engine** the tokenized state uses (a single non-tappable run) whenever the tokenized path uses it (ruby mode or selection, definitions off, non-inline). A plain React Native `<Text>` and the native paragraph engine disagree on CJK glyph weight/size at identical `fontSize`/`linePitch`/`fontWeight`/`fontFamily`, so the two states must share the engine or the page visibly changes size/weight when tokens land.
