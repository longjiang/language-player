...
### `user_subscriptions.id` / `user_acquisition.id` converted to identity (2026-08-09)

The WS-6 backfill copied Directus ids explicitly, and both columns were
originally created without `bigserial`/identity — so inserts that omitted
`id` failed with `null value in column "id" ... violates not-null constraint`
(the acquisition-survey 500). Both are now `GENERATED BY DEFAULT AS IDENTITY`
with sequences set to `max(id)` (31,120 and 29,565). The `max(id)+1` manual
allocation and `pg_advisory_xact_lock` in `utils_subscription.add_subscription`
were removed; the database now assigns ids.

### Reference tables converted to identity (2026-08-09)

The following read/reference tables were converted from bare `bigint` id
columns to `GENERATED BY DEFAULT AS IDENTITY`, with each sequence set to
`max(id)` (empty tables start at 1): `articles`, `communities`, `drills`,
`exams`, `heroes`, `languages`, `pages`, `reading`, `resources`,
`subreddits`, `tutoring_kit`, `unavailable_videos`, and `youtube_channels`
(2026-08-09).

`youtube_videos` (1,045,422 rows) was converted the same day; the alter is
metadata-only (no table rewrite), and the sequence was set to `max(id)` =
...
column assign ids. `user_subscriptions` and `user_acquisition` were converted
last; `utils_subscription.add_subscription` no longer allocates ids manually,
and `add_user_acquisition` (which already omitted `id`) now works.

No app-written tables remain pending.

In total, 19 tables were converted (13 read/reference + 6 app-written), each
with its sequence set to `max(id)` at conversion time.
...
**Resolved 2026-08-09:** `user_subscriptions.id` and `user_acquisition.id`
are `GENERATED BY DEFAULT AS IDENTITY` and `add_subscription` no longer uses
`max(id)+1` or the advisory lock. A partial unique index on
`user_subscriptions(payment_id)` (`where payment_id is not null and
payment_id <> ''`) is the idempotency key, `add_subscription` upserts with
`ON CONFLICT`, and `update_or_add_subscription` looks up an existing row by
`payment_id` first. Six pre-existing duplicate payment groups were resolved
(latest row kept, duplicates unlinked; backup:
`/tmp/subscription_idempotency_backup_20260810.json`).

The operational note documented `max(id)+1` with an advisory lock, but:

- The lock only guarded the id select inside `add_subscription`; the
...
### M4 — `user_acquisition.id` default is unverified (resolved 2026-08-09)

**Resolved 2026-08-09:** `user_acquisition.id` is now identity (sequence set
to `max(id)` = 29,565). `add_user_acquisition`'s id-less insert works;
`test_acquisition_survey_endpoint` passes against live Supabase, and the stale
Directus duplicate-key fixture was replaced with the Supabase success shape.
...
(users not present in `auth.users`) were removed, and all three tables now
have `ON DELETE CASCADE` FKs to `auth.users`. `DELETE /auth/delete-account`
...
8. **Resolved 2026-08-09:** `user_acquisition.id` (and `user_subscriptions.id`)
   ...
   and works. (M4)
9. **Resolved 2026-08-09:** deleting an `auth.users` row cascades to
   `user_subscriptions`, `user_acquisition`, and `user_id_map` via `ON DELETE
   CASCADE` FKs. (M6)
