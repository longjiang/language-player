...
### M3 — `user_subscriptions.id` allocation and webhook idempotency are not tested (partially resolved 2026-08-09)

**Partially resolved 2026-08-09:** `user_subscriptions.id` and
`user_acquisition.id` are now `GENERATED BY DEFAULT AS IDENTITY`, and
`add_subscription` no longer uses `max(id)+1` or the advisory lock. The
remaining gap below is webhook idempotency (unique key/`ON CONFLICT`), not id
allocation.
