...
`subreddits`, `tutoring_kit`, and `unavailable_videos`.

Still pending the same conversion (all app-written): `user_subscriptions`,
`user_acquisition`, `youtube_videos`, `tv_shows`, `talks`, and `phrasebooks`.
`youtube_channels` is deliberately left unchanged pending a separate review.
