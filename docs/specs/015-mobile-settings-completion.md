...
> **📐 Architecture Decision**: The list→detail settings pattern with locale-agnostic search is specified in [ADR-0015: Settings UI and Search](../adr/0015-settings-ui-and-search.md). The ADR covers the rationale, trade-off analysis, wide-screen adaptation for both platforms, and the shared `SETTINGS_SEARCH_KEYS` in `packages/shared/`. This spec implements that architecture for mobile.
