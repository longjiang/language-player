...
- **Related specs**: [SPEC-014 — Subscription & Payment System](014-subscription-payment-system.md) · [SPEC-025 — Payment E2E Testing (archived)](archive/025-payment-e2e-testing.md) · [SPEC-048 — Mobile Release Plan](048-mobile-release-plan.md) · [SPEC-067 — Google Play Release Runbook](067-google-play-release-runbook.md) · [SPEC-068 — Google Play Billing Implementation](068-google-play-billing-implementation.md) · [SPEC-023 — Mobile E2E Testing](023-mobile-e2e-testing.md) · [SPEC-039 — Full Database Migration (Supabase)](039-full-database-migration-supabase.md) · [SPEC-060 — Admin Console User Management](060-admin-console-user-management.md) · [SPEC-041 — Delete Account](041-delete-account.md)
...
- **Google Play Billing** is **not implemented** in any frontend. Android
  ...
  in Phase 3, 2026-08-10). Play Billing implementation and sandbox testing
  are Phase 3 work (the Play Console developer account was verified
  2026-08-11) — see [1.5](#15-google-play-billing-phase-3--android).
...
| Google Play Billing | N/A | N/A | — | ⬜ Phase 3 — implementation + testing (account verified 2026-08-11) |
...
SPEC-048). Phase 3 includes billing setup, implementation, and testing:

1. **Play Console developer account — ✅ verified 2026-08-11** — the existing
   account is in good standing (it was never deleted; the business-info
   renewal lapsed and reverification is now complete). Keep business info
   current; only register a new account if Play Console reports the existing
   one is fully closed.
2. **Play Console billing setup** — create the "Language Player 3" app /
   internal test track, define the Play Billing product (lifetime,
   non-consumable; product ID TBD until SPEC-014 implementation), and add
   **license testers** (Play Console → Setup → License testing). License
   testers bypass real charges.
...
phase covers both store setups plus the remaining mobile checks. C2
(subscription sync) remains deferred to Phase 5.
...
billing setup (account verified 2026-08-11), product configuration, license
testers, and test-track testing).
...
- ✅ Code inspection (2026-08-10): C5 gates (10 transcript lines / 5
  subs-search hits free; full transcript + up to 500 hits Pro), C6
...
5. **Google Play Billing is not implemented yet** — Phase 3 includes the
   Play Console billing setup (account verified 2026-08-11), implementation,
   and the G1–G5 sandbox/test-track rows (setup in
