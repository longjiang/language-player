...
- **SRS progress is per-word, not per-exercise.** `SrsProgressStore` (`packages/shared/src/types.ts:781`) is `cards: Record<l2Code, Record<wordId, SrsFields>>`, scheduled by FSRS-6 (`packages/utils/src/fsrs-scheduler.ts`). It is served by `GET /srs` and `PUT /srs/cards` (`zerotohero-python-server/routes/user_data_columns.py:174,194`), last-write-wins on `updated_at`, with a free-tier allowance on **new cards added per local day** returning `403 {"code":"srs_cap_reached"}` (`FREE_SRS_DAILY_CAP = 20`, `utils_user_data.py`; ADR-0034 Revision 1 — reviews of cards already in the deck are never capped). Sync is hardened by an outbox (ADR-0040) and the sync entity registry (`packages/utils/src/sync-entities.ts:46–50`).
- **The only "answer" table in the system** is `public.user_srs_review_log` (`user_id, rating_id, l2, word_id, rating, review_at, voided_at`), created lazily at runtime (`utils_user_data.py:389`). It records one row per rating and supports **undo by voiding** (`voidRatingId`) and **idempotency by `rating_id`**; since ADR-0034 Revision 1 it is rating history (it heals cards reset to `new`) rather than the free-tier counter, which now counts `user_srs_cards.created_at`. That shape is directly relevant here even though its subject (a word rating) is not.
...
6. **Pro gating, if applied, is content access — not a daily cap.** ADR-0034's allowance bounds how many *new cards* a free deck introduces per day (it deliberately does not gate reviewing them); textbook tasks have pre-authored answers, are never scheduled, and cost nothing per attempt, so a cap would be a different mechanism for a different purpose and must not be borrowed by analogy.
