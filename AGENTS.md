...
- **The dev-build pipeline is ARCH-028 / SPEC-076 — use it, don't improvise.** Mobile dev (Debug) builds are built, recorded, and retained through the **unified build ledger** (`docs/versioning/build-ledger.md`, one row per commit, artifacts under the gitignored `.dev-builds/`). Use:
  - `node scripts/dev-build.mjs <ios-sim|ios-device|android>` — the tracked way to make a Debug build (Metro-connected, Debug config only, refuses a dirty git tree, bakes `ip.txt` on device builds, records a ledger row, retains only the 3 most recent dev builds, `--keep N`/`--allow-dirty`/`--dry-run`).
  - `node scripts/verify-dev-build.mjs <N|latest>` — prove what a build mirrors (re-hash vs ledger, `ip.txt`, embedded commit, commit exists in git).
  - `node scripts/install-dev-build.mjs --device <udid>` — the tracked way to put a retained dev build on a physical iOS device, **LAN-change-proof**: derives the current LAN IP at run time, refuses to launch when Metro isn't reachable, installs, and launches with `-RCT_jsLocation <lan-ip>:8081` so a stale `ip.txt` (baked at build time) can't leave the app on the "No script URL provided" redbox. `--dry-run`/`--no-launch`/`--artifact` available. Docs: ARCH-028 § "Install a retained dev build onto the device (LAN-change-proof)".
  - Release/archive + number assignment are SEPARATE: `scripts/next-build.mjs` assigns the next build number (`ios.buildNumber` = `android.versionCode` = `N`, written to `packages/shared/src/version.json`), `scripts/record-build.mjs` records a consumed store build, `scripts/tag-release.mjs`, `scripts/verify-version.mjs` gates pre-upload, `scripts/bump-product-version.mjs` bumps the product version. **Never reuse a build number.** Full flow, archive-to-IPA/AAB exports, and ledger upkeep live in `docs/arch/028-local-development-runbook.md`, `docs/specs/076-versioning-strategy.md`, `docs/specs/048-mobile-release-plan.md`, and `docs/specs/067-google-play-release-runbook.md`.
- **Always check for pre-existing builds before building — ledger first, then manual searches.** Search order (per ARCH-028 §"Check for an existing build first"):
  1. **The unified build ledger** (`docs/versioning/build-ledger.md`) → the 3 most recent dev builds are at `.dev-builds/`; prove what one mirrors with `node scripts/verify-dev-build.mjs latest`.
  2. Then `.app`/`.ipa`/`.xcarchive` under `apps/mobile/ios/build`, `apps/mobile/build/devbuild` (dev-build outputs), `~/Desktop`, and Xcode DerivedData/Archives.
  3. Then CoreDevice's app-install cache: `~/Library/Containers/com.apple.CoreDevice.CoreDeviceService/Data/Library/Caches/AppInstallationBinaryDeltas/<bundle-id>/.../Stashed/*.app` — e.g. a verified `ca.zerotohero.go` debug device build from 2026-08-11 was found there after the repo/Desktop check came back empty.
  Before treating a cached build as usable, verify its embedded provisioning profile includes the target device's UDID and that it does not predate native/config changes. Keep relevant archives in the expected locations and update the ledger row/artifact when a build is consumed, so future builds/mirror checks stay accurate.
