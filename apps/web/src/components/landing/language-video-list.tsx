...
import { detectBrowserL1 } from '@/lib/docs-locale';
...
export function LanguageVideoList() {
  ...
  // Use the browser's language as L1 when it's supported, otherwise English.
  useEffect(() => {
    setL1Code(detectBrowserL1(navigator.languages?.join(',') ?? navigator.language));
  }, []);
  ...
  const sortedFamilies = [...families.entries()].sort((a, b) => a[0].localeCompare(b[0], locale));
        ...
        <section key={family}>
          ...
          <ul className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {languages.map(({ code, videoCount, name }) => (
              <li
                key={code}
                className="rounded-xl border border-border bg-surface transition-shadow hover:shadow-md dark:bg-surface-dark-secondary"
              >
                <Link href={`/${l1Code}/${code}`} className="flex items-center gap-3 px-4 py-3">
