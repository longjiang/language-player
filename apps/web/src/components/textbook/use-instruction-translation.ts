 ...
 *
 * Returns null while loading, on failure, and when translation is switched off,
 * so the caller simply renders nothing in those cases: the L2 instructions are
 * always shown and are the exercise; this is support beneath them.
 */
export function useInstructionTranslation(
  ...
  useEffect(() => {
    ...
    translateTexts({ texts: [text], l1: l1Code, l2: l2Code, apiBaseUrl: PYTHON_API_URL, signal: controller.signal })
      .then((result) => {
        if (cancelled) return;
      ...
      .catch((err) => {
        if (!cancelled) log('[LP Web] Textbook: instruction translation failed', err);
      });
