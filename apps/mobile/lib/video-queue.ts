/**
 * Watch-queue builder for the watch page (mobile).
 *
 * When a video is opened WITHOUT a grid-set queue (a deep link, a page
...
export async function buildVideoQueue(
  ...
  // saved L2 proficiency; when absent the backend returns all levels.
  try {
    ...
    const res = await fetch(`${PYTHON_API_URL}/recommend-videos?${params}`);
    if (!res.ok) return null;
    const data = await res.json();
    const queue: YouTubeVideo[] = Array.isArray(data) ? data : data?.videos ?? data?.data ?? [];
    if (queue.length === 0) return null;
