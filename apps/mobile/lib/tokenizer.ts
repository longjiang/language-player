...
import { lemmatizeLogger } from '@/lib/logger';
...
const { log, logwarn } = lemmatizeLogger;
  ...
  try {
    ...
    for (let ci = 0; ci < chunks.length; ci++) {
      ...
      for (const t of tokens) {
        ...
        // Korean phonetics are the Latin romanization (koroman) — matching
        // the server. kuromoji-ko's `reading` is Hangul and is usually
        // identical to the surface, which would suppress ruby rendering
        // (`pronunciation === word` → no ruby), so we never use it here.
        const isHangul = /[\uAC00-\uD7A3]/.test(t.surface_form);
        const pron = isHangul ? romanize(t.surface_form, 'ko') : undefined;
        if (isHangul) pronRomanized++;
        else pronNone++;
    ...
    log(`[lemmatize] 🎙 KO-PRON l2=ko tokens=${out.length} romanized=${pronRomanized} none=${pronNone} pronEqSurface=${pronEqSurface} sample=${out.slice(0, 12).map((t) => `${t.text}→${t.pronunciation ?? '∅'}`).join(', ')}`);
    return out;
  } catch (e) {
